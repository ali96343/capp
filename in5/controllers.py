...
@action("joke", method=["GET", "POST"])
@action.uses(db, session, T, "joke.html")
def joke():
    # https://github.com/apirobot/django-channels-celery-jokes

    joke1 = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
    t_vars = copy.deepcopy(html_vars)
    return locals()
...
# --------------------------------------------------------------------------------------
@action("sio_chan_post", method=["POST", "GET"])
@action.uses()
def sio_chan_post():
