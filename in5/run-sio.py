...
# https://ichi.pro/ru/kak-perezapustit-server-fastapi-s-pomos-u-scenaria-bash-264270881025116
...
# https://www.ptkom.ru/media/uploads/2019/1/prodvinutoe-ispolzovanie-celery.pdf
# https://lukaszmieczkowski.com/2021/03/29/handling-celery-events/
# https://oriolrius.cat/2013/09/30/hello-world-using-kombu-library-and-python/
# https://defpython.ru/ispolzovanie_Celery_vo_Flask_czast_2
...
def check_chmode(fnm, my_mode=0o444):
        st = os.stat(fnm)
        oct_perm = st.st_mode & 0o777
        # oct_perm = int( oct(st.st_mode), 8)  & 0o777
        # os.access(fpath, os.X_OK)
        return oct_perm != my_mode and os.chmod(fnm, my_mode)
...
def shed2list():
    ...
    for e in common_shed:
         ...
         t_file = e.split('--')[-1]
         x= [str(s) for s in t_file if s.isdigit()]
         ...
         sio_nums.add( y )
    print ('task nums (celery redis-brokers db-s): ', sorted( sio_nums) )
...
class App:
    ...
    def __init__(Z, celery_log=False, silent=False):
        Z.silent = silent
        Z.env_ok = Z.test_env()
        Z.celery_log = celery_log 
    ...
    def loader(Z,):
      ...
      for e in os.listdir(this_dir):
          if e.startswith( C.cel_files_pre, ):
            ...
            if z=='0':
               ...
               continue
            cmd = f'celery -A {C.APPS_DIR}.{C.P4W_APP}.{C.cel_files_pre}{z} worker -Q {C.cel_queue_pre}{z} -B -s {C.shed_path}-{z}'  # -D  --detach
            #cmd_log = '--loglevel=warning -f celery.logs'
            if Z.celery_log:
                cmd_log = f' -E --loglevel=warning -f {C.shed_path}-{z}.logs -D'
                cmd += cmd_log
            od[y] = cmd
      ...
      if chan_sio_file:
            ...
            #cmd = f"nohup python {C.APPS_DIR}/{C.P4W_APP}/{C.SIO_FILE} &"
            od[  0 ] =cmd
...
@click.option(
    "--celery_log", "-l", is_flag=True, default=False, type=click.BOOL, help=f"add celery log file"
)

def cli_main(restart, stop, killall_sio_and_celery, celery_log):
